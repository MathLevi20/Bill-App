...
const FinancialChart: React.FC<FinancialChartProps> = ({ bills, title }) => {
    ...
    datasets: [
      {
        ...
        data: sortedBills.map(bill => bill.totalValueWithoutGD),
      ...
      {
        label: 'GD Economy (R$)',
        data: sortedBills.map(bill => bill.gdSavings),
  ...
  return (
    <ChartContainer>
      <ChartTitle>
        <FaMoneyBillWave className="inline-block mr-2 text-primary" /> {title}
      </ChartTitle>
      {bills.length === 0 ? (
